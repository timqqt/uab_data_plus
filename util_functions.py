...
def uabUtilAllTypeLoad(fileName):
    ...
    try:
        if fileName[-3:] != 'npy':
            if '.mat' in fileName:
                outP = scipy.io.loadmat(fileName)
                outP = outP[list(outP.keys())[-1]]
            else:
                outP = scipy.misc.imread(fileName)
            # outP = imageio.imread(fileName)
        else:
            outP = np.load(fileName) # is it OK for .mat file?

        return outP
    except Exception: # so many things could go wrong, can't be more specific.
        print('The name of problem file is ', fileName)
...
def uabUtilAllTypeSave(fileName, variable_to_save):
    ...
    if fileName[-3:] != 'npy':
        ...
        if '.mat' in fileName:
            if 'RGB' in fileName:
                scipy.io.savemat(fileName, {'RGB': variable_to_save})
            elif 'GT' in fileName:
                scipy.io.savemat(fileName, {'GT': variable_to_save})
        else:
...
def image_summary_only_pred_and_GT(image, truth, prediction, img_mean=np.array((0, 0, 0), dtype=np.float32)):
    """
    By Fanjie
    Only summarize GT % Pred in tensorboard
    :param image:
    :param truth:
    :param prediction:
    :param img_mean:
    ...
    return np.concatenate([truth_img, pred_img], axis=2)
