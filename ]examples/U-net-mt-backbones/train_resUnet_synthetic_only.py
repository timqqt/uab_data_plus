...
n_train = 2500                  # number of samples per epoch
...
model =  uabMakeNetwork_U_net.Unet_MTbackbones({'X': X, 'Y': y},
...
# create collection
blCol = uab_collectionFunctions.uabCollection('')
...
file_list_train = uabCrossValMaker.make_file_list_by_key(idx, file_list, [i for i in range(0, 1925)], filter_list = [''])
file_list_valid = uabCrossValMaker.make_file_list_by_key(idx, file_list, [i for i in range(1925, 2310)],  filter_list = [''])
