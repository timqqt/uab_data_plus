...
from bohaoCustom import uabMakeNetwork_U_net_multi_backbones as uabMakeNetwork_UNet     # you can also import other models from bohaoCustom
...
input_size = [288, 288]         # input size to NN, same as extracted patch size, no need to be the same as training
...
model_dir = r'/work/fk43/Models/resnet34U_net_Joint_training_syn_and_deepglobe_resUnet_ratio_6_1_3_PS(288, 288)_BS7_EP100_LR5e-05_DS50_DR0.1_SFN32'
# make the model, same as training
X = tf.placeholder(tf.float32, shape=[None, input_size[0], input_size[1], 3], name='X')
y = tf.placeholder(tf.int32, shape=[None, input_size[0], input_size[1], 1], name='y')
mode = tf.placeholder(tf.bool, name='mode')
model = uabMakeNetwork_UNet.Unet_MTbackbones({'X':X, 'Y':y},
                                            trainable=mode,
...
file_list_valid = uabCrossValMaker.make_file_list_by_key(
    idx, file_list, [i for i in range(1, 6)],  filter_list=['bellingham', 'bloomington', 'sfo', 'tyrol-e', 'innsbruck',  'vienna_s'])
file_list_valid_truth = uabCrossValMaker.make_file_list_by_key(
    idx_truth, file_list_truth, [i for i in range(1, 6)], filter_list=['bellingham', 'bloomington', 'sfo', 'tyrol-e', 'innsbruck', 'vienna_s'])
